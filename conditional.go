...
// ConditionalTransaction is a convenience wrapper for one durable,
// idempotent v2 transaction. Callers that need safe recovery after an
// indeterminate acknowledgement should retain a value from
// NewConditionalTransactionRequest and call ExecuteConditionalTransaction.
// Condition rejection is returned as a typed conflict result, request-id
// rebinding is CodeNativeConflict, and a possibly-applied acknowledgement
// failure is CodeResultIndeterminate.
...
// ExecuteConditionalTransaction executes a previously sealed request. Reuse
// this exact value for receipt recovery if the acknowledgement is
// indeterminate.
func (db *DB) ExecuteConditionalTransaction(request ConditionalTransactionRequest) (ConditionalTransactionResult, error) {
	...
	data, err := db.execute("storage", "conditional_batch", built.wire)
	if err != nil {
		return ConditionalTransactionResult{}, err
	}
...
// ConditionalTransactionReceiptFor resolves an exact request after an
// indeterminate acknowledgement. A successful indeterminate lookup means
// consensus has not yet produced a final transaction receipt; it does not
// authorize retrying with another payload.
