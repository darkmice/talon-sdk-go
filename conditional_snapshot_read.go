...
// ConditionalSnapshotGet reads a bounded key set from one Core MVCC snapshot
// and verifies request identity, ordering, absent/empty distinction, bounds,
// and Core's response digest.
func (db *DB) ConditionalSnapshotGet(request ConditionalSnapshotReadRequest) (ConditionalSnapshotReadResult, error) {
	...
	data, err := db.execute("storage", "conditional_snapshot_get", wire)
	if err != nil {
		return ConditionalSnapshotReadResult{}, err
	}
